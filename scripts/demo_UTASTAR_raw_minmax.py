#!/usr/bin/env python3
"""
Demo UTA-STAR implementation - RAW MIN-MAX NORMALIZATION ONLY.
Uses combined demo selections as preferred parcels and tests on raw min-max datasets.

Focuses only on raw min-max normalized datasets:
- raw_minmax_all (All parcels with raw min-max normalization)
- raw_minmax_wui (WUI parcels with raw min-max normalization)
"""
...
OUTPUTS_DIR = BASE_PATH / "extras" / "outputs" / "demo_utastar_raw_minmax"
OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)
SHAPEFILE_OUTPUT_DIR = DEMO_DATA_DIR / "outputs_raw_minmax"
...
# Dataset configurations - RAW MIN-MAX ONLY
DATASET_CONFIGS = [
    {
        'name': 'raw_minmax_all',
...
# Selection paths (COMBINATION: San Antonio Creek + Above E Mtn Drive + Mission Canyon Montrose)
SELECTION_PATHS = [
    'selections/San_antonio_creek/fire_risk_local_raw_minmax_spatial_wui_parcels.shp',
...
def identify_parcel_subsets(parcel_ids: List[str]) -> Dict[str, str]:
    ...
    subset_mappings = [
        ('san_antonio_creek', 'selections/San_antonio_creek/fire_risk_local_raw_minmax_spatial_wui_parcels.shp'),
        ('above_E_mtn_drive', 'selections/above_E_mtn_drive/fire_risk_local_raw_minmax_spatial_parcels.shp'),
        ('mission_canyon_montrose', 'selections/mission_canyon_montrose_quantile/fire_risk_local_quantile_spatial_parcels.shp')
    ]
...
def save_utility_scores_shapefile(gdf: gpd.GeoDataFrame, scores: np.ndarray, parcel_ids: List[str],
    ...
    Args:
        ...
        dataset_name: Name of the dataset (e.g., 'raw_minmax_all')
...
# -----------------------------------------------------------------------------
# SECTION: UTA-STAR Implementation (identical to quantile version)
# -----------------------------------------------------------------------------
...
def plot_utility_functions_terminal(utility_result: Dict, dataset_name: str = "") -> None:
    ...
    suptitle = "Demo UTA-STAR Raw Min-Max: Utility functions per criterion"
    if dataset_name:
...
def generate_score_map(parcels: gpd.GeoDataFrame, scores: np.ndarray, preferred_idx: List[int],
    ...
    try:
        ...
        try:
            selection_areas = ['san_antonio_creek', 'above_E_mtn_drive', 'mission_canyon_montrose']
...
def main():
    """Main execution function for demo UTA-STAR analysis - RAW MIN-MAX ONLY."""
    print("="*80)
    print("DEMO UTA-STAR: RAW MIN-MAX NORMALIZATION ONLY")
    print("="*80)
    ...
    # Test each raw min-max dataset configuration
                ...
                if utility_result:
                    ...
                    try:
                        ...
                        generate_score_map(
                            ...
                            title=f"Demo UTA-STAR Raw Min-Max Scores — {config['name']}",
                            subtitle=f"alpha={alpha} (segments={alpha})",
                            output_name=f"demo_raw_minmax_map_{config['name']}_alpha{alpha}",
                            ranks=ranking_u,
                            alpha_value=alpha
                        )
                    except Exception as e:
            ...
            if weight_result:
                ...
                try:
                    ...
                    generate_score_map(
                        ...
                        title=f"Demo Linear Weights Raw Min-Max Scores — {config['name']}",
                        subtitle="Linear weighted sum (alpha=1)",
                        output_name=f"demo_raw_minmax_map_linear_{config['name']}",
    ...
    if all_results:
        print("\n" + "="*80)
        print("FINAL SUMMARY: RAW MIN-MAX DATASETS ONLY")
        print("="*80)
        ...
        # Save summary CSV
        csv_path = OUTPUTS_DIR / 'demo_utastar_raw_minmax_summary.csv'
        try:
    ...
    print(f"\nDemo UTA-STAR Raw Min-Max analysis complete!")
    print(f"Preferred parcels analyzed: {len(preferred_ids)}")
    print(f"Raw Min-Max datasets tested: {len(DATASET_CONFIGS)}")
