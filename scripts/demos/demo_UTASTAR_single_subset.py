#!/usr/bin/env python3
"""
Demo UTA-STAR implementation - SINGLE SUBSET (Mission Canyon Montrose).
Uses only Mission Canyon Montrose as preferred parcels and tests on both datasets.

Focuses on both datasets:
- quantile_all (All parcels with quantile normalization)
- quantile_wui (WUI parcels with quantile normalization)
"""
...
OUTPUTS_DIR = BASE_PATH / "extras" / "outputs" / "demo_utastar_single_subset"
OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)
SHAPEFILE_OUTPUT_DIR = DEMO_DATA_DIR / "outputs_single_subset"
...
# Dataset configurations - QUANTILE ONLY
DATASET_CONFIGS = [
    {
        'name': 'quantile_all',
...
# Single selection path - Mission Canyon Montrose only
SELECTION_PATH = 'selections/mission_canyon_montrose_quantile/fire_risk_local_quantile_spatial_parcels.shp'
...
def load_mission_canyon_selection() -> set[str]:
    """
    Load Mission Canyon Montrose selection and return set of parcel IDs.
    """
    print("Loading Mission Canyon Montrose selection...")
    
    full_path = DEMO_DATA_DIR / SELECTION_PATH
    try:
        sel_gdf = gpd.read_file(full_path)
        preferred_ids = set(sel_gdf['parcel_id'].astype(str))
        print(f"  Mission Canyon Montrose: {len(preferred_ids):,} parcels")
...
def main():
    ...
    print("="*80)
    print("DEMO UTA-STAR: SINGLE SUBSET (MISSION CANYON MONTROSE)")
    print("="*80)
    
    # Load Mission Canyon selection (preferred parcels)
    preferred_ids = load_mission_canyon_selection()
    if not preferred_ids:
        print("Error: No preferred parcels found in Mission Canyon selection")
        return
    ...
    if all_results:
        print("\n" + "="*80)
        print("FINAL SUMMARY: SINGLE SUBSET (MISSION CANYON)")
        print("="*80)
