#!/usr/bin/env python3
"""
Demo UTA-STAR implementation for fire risk analysis.
Uses combined demo selections as preferred parcels and tests on multiple datasets.

Key differences from main UTASTAR.py:
- Fixed preferred set: 271 parcels from 3 demo area selections
- Full reference set: ALL non-preferred parcels (no sampling)
- Multiple datasets: quantile vs raw_minmax normalization, all vs wui parcels
- Same visual outputs: utility plots, score maps, ranking tables
"""
...
OUTPUTS_DIR = BASE_PATH / "extras" / "outputs" / "demo_utastar"
OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)
SHAPEFILE_OUTPUT_DIR = DEMO_DATA_DIR / "outputs"
...
# Alpha values for piecewise utilities (same as original)
...
DATASET_CONFIGS = [
    {
        'name': 'quantile_all',
        'path': 'fire_risk_local_quantile_all_parcels/fire_risk_local_quantile_all_parcels.shp',
        'description': 'All parcels with quantile normalization'
    },
    {
        'name': 'quantile_wui', 
        'path': 'fire_risk_local_quantile_wui_parcels/fire_risk_local_quantile_wui_parcels.shp',
        'description': 'WUI parcels with quantile normalization'
    },
    {
        'name': 'raw_minmax_all',
        'path': 'fire_risk_local_raw_minmax_all_parcels/fire_risk_local_raw_minmax_all_parcels.shp', 
        'description': 'All parcels with raw min-max normalization'
    },
    {
        'name': 'raw_minmax_wui',
        'path': 'fire_risk_local_raw_minmax_wui_parcels/fire_risk_local_raw_minmax_wui_parcels.shp',
        'description': 'WUI parcels with raw min-max normalization'
    }
]

# Selection paths
SELECTION_PATHS = [
    'selections/painted_cave_quantile/fire_risk_local_quantile_spatial_parcels.shp',
    'selections/mission_canyon_montrose_quantile/fire_risk_local_quantile_spatial_parcels.shp', 
    'selections/above_E_mtn_drive/fire_risk_local_raw_minmax_spatial_parcels.shp'
]
...
def load_demo_selections() -> set[str]:
    ...
    Returns the union of all parcel_ids from the 3 selection areas.
    """
    print("Loading demo area selections...")
    all_preferred_ids = set()
    
    for sel_path in SELECTION_PATHS:
        full_path = DEMO_DATA_DIR / sel_path
        try:
...
def load_dataset(config: Dict) -> gpd.GeoDataFrame:
    ...
    dataset_path = DEMO_DATA_DIR / "dataset" / config['path']
...
def prepare_demo_data(gdf: gpd.GeoDataFrame, preferred_ids: set[str]) -> Tuple[np.ndarray, List[str], List[int], List[int]]:
    ...
    # Get indices for preferred and non-preferred parcels
    ...
    print(f"  Non-preferred parcels: {len(non_preferred_idx):,}")
...
def stratified_spatial_sample(parcels_gdf: gpd.GeoDataFrame, preferred_ids: List[str], target_size: int = 10000, seed: int = 42) -> List[int]:
    """
...
def identify_parcel_subsets(parcel_ids: List[str]) -> Dict[str, str]:
    ...
    subset_mappings = [
        ...
        ('mission_canyon_montrose_quantile', 'selections/mission_canyon_montrose_quantile/fire_risk_local_quantile_spatial_parcels.shp'),
...
# -----------------------------------------------------------------------------
# SECTION: UTA-STAR Implementation (copied from original with minor tweaks)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# SECTION: Visualization Functions (copied from original)
# -----------------------------------------------------------------------------
...
def plot_utility_functions_terminal(utility_result: Dict, dataset_name: str = "") -> None:
    ...
    suptitle = "Demo UTA-STAR: Utility functions per criterion"
    if dataset_name:
...
def generate_score_map(parcels: gpd.GeoDataFrame, scores: np.ndarray, preferred_idx: List[int],
    ...
    try:
        ...
        # Mark preferred parcels centroid with a star
        try:
            preferred_geoms = parcels.loc[parcels['is_preferred'], 'geometry']
            if len(preferred_geoms) > 0:
                centroid = preferred_geoms.unary_union.centroid
...
def main():
    """Main execution function for demo UTA-STAR analysis."""
    print("="*80)
    print("DEMO UTA-STAR: FIRE RISK ANALYSIS WITH COMBINED SELECTIONS")
    print("="*80)
                ...
                if utility_result:
                    ...
                    try:
                        ...
                        generate_score_map(
                            ...
                            title=f"Demo UTA-STAR Scores — {config['name']}",
                            subtitle=f"alpha={alpha} (segments={alpha})",
                            output_name=f"demo_map_{config['name']}_alpha{alpha}",
                            ranks=ranking_u,
                            alpha_value=alpha
                        )
                    except Exception as e:
            ...
            if weight_result:
                ...
                try:
                    ...
                    generate_score_map(
                        ...
                        title=f"Demo Linear Weights Scores — {config['name']}",
                        subtitle="Linear weighted sum (alpha=1)",
                        output_name=f"demo_map_linear_{config['name']}",
    ...
    if all_results:
        ...
        # Save summary CSV
        csv_path = OUTPUTS_DIR / 'demo_utastar_summary.csv'
        try:
    ...
    print(f"\nDemo UTA-STAR analysis complete!")
    print(f"Preferred parcels analyzed: {len(preferred_ids)}")
    print(f"Datasets tested: {len(DATASET_CONFIGS)}")
